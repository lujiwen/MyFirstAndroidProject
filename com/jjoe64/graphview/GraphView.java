		...
		/*paint.setDither(true) ;*/
	      ...
	      */
			...
			if (labelTextHeight == null || horLabelTextWidth == null) {
			 ...
			 try
			 {
				String testLabel = formatLabel(testX, true); // bk    
			 ...
			 catch(Exception e )
			 {
				...
				labelTextHeight = Integer.valueOf(textBounds.height());
				horLabelTextWidth = Integer.valueOf(textBounds.width());
			}
			assert(labelTextHeight != null);
			border += labelTextHeight.floatValue();
			graphBorder = border ;
			float graphheight = height - (2 * border);
			this.graphheight = graphheight ;
			graphwidth = width;
			
/*			Log.e("ondrawborder", border+"") ;
			Log.e("ondrawgraphwidth", width+"") ;
			Log.e("ondrawheight", height+"") ;
			Log.e("ondrawgraphheight", graphheight+"");	*/				
...
/*				Log.e("border",border+"");*/
			...
			try{
				for (int i=0; i<graphSeries.size(); i++) {				 	
					if(graphSeries.get(i).style.isDash)
					{
						drawDashSeries(canvas, _values(i), graphwidth, graphheight, border, minX, minY, diffX, diffY, horstart, graphSeries.get(i).style);
					}
					else 
					{
...
*/
	...
	static public class GraphViewData implements GraphViewDataInterface {
		...
		public GraphViewData(double valueX, double valueY,boolean isdashPt) {
			super();
			this.valueX = valueX;
			this.valueY = valueY;
			this.isDashPoint = isdashPt ;
		}
		@Override
		public double getX() {
			return valueX;
		...
		protected void onDraw(Canvas canvas) {
			...
			if (labelTextHeight == null || verLabelTextWidth == null) {
				...
				double testY = ((getMaxY()-getMinY())*0.783)+getMinY();
				 try {
					 String testLabel = formatLabel(testY, false);   //bk
					 paint.getTextBounds(testLabel, 0, testLabel.length(), textBounds);
				} catch (Exception e) {
	...
	private static String[] horlabels;
	...
	public float getGraphHeight()
	{
		return graphViewContentView.getGraphHeight();
	}
	public float getGraphWidth()
	{ 
		return graphViewContentView.getGraphWidth();
	}
	public float getGraphBorder()
	{
		return graphViewContentView.graphBorder ;
	}
	 ...
	 * draw a dashLine 
	 * */
	public void addDashSeries(GraphViewSeries series) //虚线
	{
		series.addGraphView(this);
		graphSeries.add(series);
		series.getStyle().isDash = true ;
		redrawAll();
	}
	...
	abstract protected void drawDashSeries(Canvas canvas, GraphViewDataInterface[] values, float graphwidth, float graphheight, float border, double minX, double minY, double diffX, double diffY, float horstart, GraphViewSeriesStyle style);
	...
	protected String formatLabel(double value, boolean isValueX) {
		...
		if (numberformatter[i] == null) {
			...
			if (highestvalue - lowestvalue < 0.1) {
				...
				numberformatter[i].setMaximumFractionDigits(0);  // bk
			}
		}
		return numberformatter[i].format(value);  // bk
	}
...
/*		invalidate();  
	 ...
	 */
	public void setHorizontalLabels(String[] horlabel ) {
		staticHorizontalLabels = horlabel != null;
		horlabels = horlabel;
		//redrawAll() ;
	}
	public  void  resetHorizontalLabels(String[]  newHorlabels)
	{
			horlabels = newHorlabels ;
			Log.e("GraphView", horlabels[0]+","+horlabels[1]+","+horlabels[2]+","+horlabels[3] );
			redrawAll() ; 		
	} 	
	/**
	 * legend position
